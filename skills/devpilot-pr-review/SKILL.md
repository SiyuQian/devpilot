...
- Close with the HTML comment metadata block exactly as shown below. The block is required on every posted review so readers can tell which devpilot version produced it.
...
The skill itself is unversioned. Every posted review carries the **devpilot binary version** instead, so readers can attribute the review to a specific release. Resolve it at post time via `devpilot --version` (the binary prints e.g. `devpilot version v0.12.2`; take the `vX.Y.Z` token). If the command is unavailable, fall back to the string `unknown`.
...
The disclaimer blockquote is **required** and must be posted verbatim (translate into the PR's language if the review is not in English, but keep the meaning — automated, not authoritative, human judgment required).
...
| "The version comment is noise, I'll drop it." | The `<!-- devpilot-pr-review (devpilot vX.Y.Z) -->` block is required so readers can attribute the review to a specific devpilot release. |
