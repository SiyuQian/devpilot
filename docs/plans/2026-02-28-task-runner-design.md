...
- Runner continues to the next card — never blocks on a single failure
...
1. Initialize
   ...
   b. Resolve board name → board ID
   c. Resolve list names → list IDs (Ready/In Progress/Done/Failed)

2. Loop
   a. GET cards from Ready list
   b. No cards → sleep interval → continue
   c. Take first card
   d. Move card → In Progress
   ...
   i. Check exit code:
      - Success (0):
        ...
        - Move card → Done
        - Comment: ✅ + PR link + duration
      - Failure (non-0):
        - Move card → Failed
        - Comment: ❌ + error summary + log path
   j. git checkout main && git pull
   k. --once? → exit : continue loop
```
...
| `internal/runner/runner.go` | Main loop: poll → dispatch → handle result |
