...
**Goal:** Remove all dead MCP server artifacts and rewrite the Trello skill to use direct curl calls with devpilot-managed credentials.

**Architecture:** Delete `.mcp.json`, clean MCP references from `.claude/settings.local.json`, and rewrite the Trello skill SKILL.md to instruct Claude to read credentials from `~/.config/devpilot/credentials.json` and make direct curl calls to the Trello REST API.
...
- Line 8: `"Bash(cd /Users/siyu/Works/github.com/siyuqian/devpilot/mcps/trello-mcp-server && npm install 2>&1)"`
- Line 9: `"Bash(npm run:*)"`
...
Credentials are read from ~/.config/devpilot/credentials.json
(managed by devpilot login trello)."
```
