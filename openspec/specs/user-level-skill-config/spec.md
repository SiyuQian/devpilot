...
The system SHALL use the existing `project.Load` and `project.Save` functions with the user config directory to read and write user-level skill entries. The user-level `.devpilot.yaml` uses the same format as the project-level config.
...
- **AND** calls `cfg.UpsertSkill(entry)` with the skill's name, source, version, and installedAt
