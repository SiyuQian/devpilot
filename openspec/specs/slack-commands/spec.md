...
- **THEN** the system SHALL return an error: "bot is not a member of the channel, run: /invite @devpilot in the channel"
...
#### Scenario: First match resolution
- **WHEN** the system iterates through channels from `conversations.list`
- **THEN** it SHALL return the first channel whose name exactly matches the given name
