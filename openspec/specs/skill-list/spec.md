...
The system SHALL display only installed skills (from both project-level and user-level configs) when the user runs `devpilot skill list --installed`. Output SHALL include NAME, DESCRIPTION, VERSION, LEVEL columns. Project-level skills are listed first, then user-level skills.
...
- **THEN** the system prints a table with columns: NAME, DESCRIPTION, VERSION, LEVEL
